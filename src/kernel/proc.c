...
void init_proc(Proc* p) // 初始化进程
{
    ...
    p->ucontext = kalloc_page() + PAGE_SIZE - sizeof(UserContext);   // 分配用户进程上下文
    p->ucontext->sp_el0 = round_up((u64)p->ucontext, PAGE_SIZE);     // 设置用户进程栈指针
...
int kill(int pid)
{
    if (pid <= 0 || pid >= MAX_PID) return -1; // 检查 PID 是否有效
